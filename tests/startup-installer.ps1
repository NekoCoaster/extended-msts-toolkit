...
foreach($source in @($BaseExe,$WidescreenExe)){foreach($laa in @($false,$true)){
 ...
 foreach($bits in 1..7){
  $v=($bits -band 1) -ne 0;$l=($bits -band 2) -ne 0;$fps=($bits -band 4) -ne 0
  Invoke-Options $file $false $false $false -VerboseLoading $v -StartupLog $l -UnlockFPS $fps -Confirm:$false | Out-Null
  $m=Get-CrawlInstallation $file
  if($m.verboseLoading -ne $v -or $m.startupLog -ne $l -or $m.unlockFPS -ne $fps){throw 'Startup flags mismatch'}
  $ini=[IO.File]::ReadAllText((Join-Path $dir 'NEMT/native.ini'))
  foreach($pair in @(@('VerboseLoading',$v),@('WriteLog',$l),@('UnlockFPS',$fps))){if(-not $ini.Contains(($pair[0]+'='+$pair[1].ToString().ToLowerInvariant()))){throw 'INI mismatch'}}
...
'PASS 28 startup feature combinations across four variants, opt-in settings, no installer log writes and unchanged EXE.'
