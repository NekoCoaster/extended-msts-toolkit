...
foreach($key in @('UseSystemMovieDecoder','RestoreMovieFocus','SeparateMovieWindow')){if(-not $text.Contains("$key=true")){throw 'Missing default movie fix'}}
$text=$text.Replace('SeparateMovieWindow=true','SeparateMovieWindow=false').Replace('UseSystemMovieDecoder=true','UseSystemMovieDecoder=false').Replace('RestoreMovieFocus=true','RestoreMovieFocus=false');[IO.File]::WriteAllText($ini,$text)
Invoke-Options $file $false $false $false -WindowFeatures $true -Confirm:$false | Out-Null
$text=[IO.File]::ReadAllText($ini)
foreach($key in @('UseSystemMovieDecoder','RestoreMovieFocus','SeparateMovieWindow')){if(-not $text.Contains("$key=false")){throw 'Movie comparison setting lost'}}
