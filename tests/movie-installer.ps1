...
foreach($skip in @($false,$true)){foreach($focus in @($false,$true)){
 Invoke-Options $file $false $false $false -WindowFeatures $true -SkipStartupMovie $skip -RestoreMovieFocus $focus -Confirm:$false | Out-Null
 $ini=Join-Path $dir 'NEMT/settings.ini';$text=[IO.File]::ReadAllText($ini)
 if(-not $text.Contains("SkipStartupMovie=$($skip.ToString().ToLowerInvariant())") -or -not $text.Contains("RestoreMovieFocus=$($focus.ToString().ToLowerInvariant())")){throw 'Movie checkbox value mismatch'}
...
Invoke-Options $file $false $false $false -RestoreMovieFocus $true -Confirm:$false | Out-Null
if(-not (Test-Path -LiteralPath (Join-Path $dir 'DINPUT.dll'))){throw 'Focus-only installation failed'}
...
'PASS movie option combinations, native-rendering config, standalone movie features, uninstall and unchanged executable.'
