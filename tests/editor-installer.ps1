...
foreach($source in @($BaseExe,$WidescreenExe)){foreach($laa in @($false,$true)){
 ...
 foreach($bits in 1..7){
  ...
  Invoke-Options $file $false $false $false -FixCabDials $false -EditorWindows $windows -FreeEditorTools $tools -SmoothEditorAudio $audio -Confirm:$false | Out-Null
  ...
  foreach($pair in @(@('ResizableViewports',$windows),@('FreeToolWindows',$tools),@('SmoothIdleAudio',$audio))){if(-not $text.Contains($pair[0]+'='+$pair[1].ToString().ToLowerInvariant())){throw 'Editor setting mismatch'}}
  if($manifest.editorWindows -ne $windows -or $manifest.freeEditorTools -ne $tools -or $manifest.smoothEditorAudio -ne $audio){throw 'Editor manifest mismatch'}
 }
...
'PASS editor options: all eight combinations across four executable variants, independent installation, persistence and uninstall; EXE unchanged.'
