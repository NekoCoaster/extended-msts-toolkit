...
foreach($source in @($BaseExe,$WidescreenExe)){foreach($laa in @($false,$true)){
 ...
 foreach($bits in 1..31){
  $windows=($bits -band 1) -ne 0;$tools=($bits -band 2) -ne 0;$audio=($bits -band 4) -ne 0
  $keys=($bits -band 8) -ne 0;$pan=($bits -band 16) -ne 0
  Invoke-Options $file $false $false $false -FixCabDials $false -EditorWindows $windows -FreeEditorTools $tools -SmoothEditorAudio $audio -SwapEditorKeys $keys -UnlimitedEditorPan $pan -Confirm:$false | Out-Null
  ...
  foreach($pair in @(@('ResizableViewports',$windows),@('FreeToolWindows',$tools),@('SmoothIdleAudio',$audio),@('SwapArrowKeys',$keys),@('UnlimitedMousePan',$pan))){if(-not $text.Contains($pair[0]+'='+$pair[1].ToString().ToLowerInvariant())){throw 'Editor setting mismatch'}}
  if($manifest.editorWindows -ne $windows -or $manifest.freeEditorTools -ne $tools -or $manifest.smoothEditorAudio -ne $audio -or $manifest.swapEditorKeys -ne $keys -or $manifest.unlimitedEditorPan -ne $pan){throw 'Editor manifest mismatch'}
...
'PASS editor options: all thirty-two combinations across four executable variants, independent installation, custom binding persistence and uninstall; EXE unchanged.'
