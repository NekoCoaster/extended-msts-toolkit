...
static B *load_probe(const char *path){
 ...
 for(j=0;j<6;j++){U original=h[j].address;h[j].address=(U)code+original-0x7060b0;if(j!=2)*(U*)(h[j].replacement+1)=(U)stub+(j>=3?64:0)-h[j].address-5;}
...
int main(int argc,char **argv){
 ...
 for(logging=0;logging<2;logging++)for(i=0;i<9;i++){U result;startup_log=logging;scenario=i;device_warned=notices=owner_queries=0;creates=enums=releases=surfaces=0;result=probe(&d3d,&requested_kind,(void*)0x5678,(void*)0x9abc);
 assert(result==(i==0||i==2));assert(enums==(i==0||i==2||i==8));if(i==2||i==3)assert(creates==2&&surfaces==2);if(i==1||i==4||i==5)assert(creates==1&&enums==0);if(i==6||i==7)assert(creates==0);assert(releases>0||i==6);assert(notices==(i==1||i==3||i==4||i==5));assert(owner_queries==(logging?creates:0));
 }
