...
static void *draw_v[7],*surface_v[23],*d3d_v[5],*device_v[5];
...
static HRESULT WINAPI create_surface(void *self,void *desc,void **out,void *outer){surfaces++;if(scenario==6)return E_FAIL;*out=&surface;return 0;}
...
static B *load_probe(const char *path){
 FILE *f=fopen(path,"rb");B *file,*code,*stub;long bytes;U pe,table,i,j,rva,raw,size;DWORD old;Hook h[3];assert(f);
 ...
 for(j=0;j<3;j++){U original=h[j].address;h[j].address=(U)code+original-0x7060b0;if(j<2)*(U*)(h[j].replacement+1)=(U)stub-h[j].address-5;}
 assert(prepare_hooks(h,3)&&install_hooks(h,3));assert(VirtualProtect(code,4096,PAGE_EXECUTE_READ,&old));FlushInstructionCache(GetCurrentProcess(),code,4096);return code;
}
int main(int argc,char **argv){
 typedef U (__fastcall *Probe)(void*,const GUID*,void*,void*);Probe probe;int i,logging;assert(argc==2);
 draw_v[6]=create_surface;surface_v[2]=release;surface_v[22]=description;d3d_v[4]=create_device;device_v[2]=release;device_v[4]=enumerate;
 probe=(Probe)load_probe(argv[1]);
 for(logging=0;logging<2;logging++)for(i=0;i<9;i++){U result;startup_log=logging;scenario=i;creates=enums=releases=surfaces=0;result=probe(&d3d,&requested_kind,(void*)0x5678,(void*)0x9abc);
 assert(result==(i==0||i==2));assert(enums==(i==0||i==2||i==8));if(i==2||i==3)assert(creates==2&&surfaces==2);if(i==1||i==4||i==5)assert(creates==1&&enums==0);if(i==6||i==7)assert(creates==0);assert(releases>0||i==6);
 }
 puts("PASS original native device probe: success, creation failures, null-success, pixel fallback, enumeration failure and cleanup; patched stdcall/flags exercised.");return 0;
}
