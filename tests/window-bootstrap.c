...
static int calls;
static LPSTR WINAPI compatibility_command_line(void){calls++;return "train.exe -vm:bw,1280,720,32";}
int main(void){
 ...
 *slot=(LONG)compatibility_command_line;assert(*slot!=(LONG)GetCommandLineA);
 ...
 assert(normalize_vm(original_command_line(),translated)==2);assert(!strcmp(translated,"train.exe -vm:w,1280,720,32"));
