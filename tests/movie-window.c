...
static U __fastcall open_child(HWND h,const char *path){assert(GetWindowLongA(h,GWL_STYLE)&WS_CHILD);assert(GetParent(h)==*(HWND*)MOVIE_ADDR(0x82813a));return 1;}
int main(void){
 ...
 *(HWND*)MOVIE_ADDR(0x82813a)=owner;separate_movie_window=1;movie_show_original=show;movie_send=message;
 movie_original_open=open_child;assert(movie_open(child,"startup.mpg")==1);assert(GetWindowLongA(child,GWL_STYLE)&WS_CHILD);
 assert(movie_show(child,SW_SHOW)==123&&shows==1&&destinations==1);assert(!(GetWindowLongA(child,GWL_STYLE)&WS_CHILD));assert(!(GetWindowLongA(child,GWL_STYLE)&WS_BORDER));assert(GetWindow(child,GW_OWNER)==owner);assert(!(GetWindowLongA(child,GWL_EXSTYLE)&WS_EX_TOPMOST));
 GetClientRect(child,&c);GetWindowRect(child,&r);GetClientRect(owner,&o);ClientToScreen(owner,&p);
