...
static BOOL CALLBACK select_secondary(HMONITOR monitor,HDC dc,LPRECT rect,LPARAM data){
 MONITORINFOEXA info;memset(&info,0,sizeof(info));info.mi.cbSize=sizeof(info);
 if(GetMonitorInfoA(monitor,(MONITORINFO*)&info)&&!(info.mi.dwFlags&MONITORINFOF_PRIMARY)){
  strcpy(window_monitor,info.szDevice);return FALSE;
 }return TRUE;
}
 ...
 *(HWND*)WINDOW_ADDR(0x82813a)=h;original_window_pos=SetWindowPos;original_show=ShowWindow;
 ...
 /* Same-size placement must not undo the selected borderless anchor. */
 EnumDisplayMonitors(NULL,NULL,select_secondary,0);
 center_for(h,800,600,&x,&y);
 assert(toolkit_window_pos(h,NULL,30,40,0,0,SWP_NOSIZE|SWP_NOZORDER|SWP_NOACTIVATE));GetWindowRect(h,&r);assert(r.left==x&&r.top==y);
 assert(toolkit_window_pos(h,NULL,0,0,800,600,SWP_NOZORDER|SWP_NOACTIVATE));GetWindowRect(h,&r);assert(r.left==x&&r.top==y);dimensions(h,800,600);
 /* A later show also repairs displacement by another caller. */
 SetWindowPos(h,NULL,30,40,0,0,SWP_NOSIZE|SWP_NOZORDER|SWP_NOACTIVATE);
 toolkit_show(h,SW_SHOWNOACTIVATE);GetWindowRect(h,&r);assert(r.left==x&&r.top==y);
 ...
 printf("Borderless anchor tested on %s\n",*window_monitor?window_monitor:"primary (no secondary connected)");
 DestroyWindow(h);puts("PASS native Win32 border removal, client dimensions, same-size/show anchoring, resize centering and bordered movement.");return 0;
}
