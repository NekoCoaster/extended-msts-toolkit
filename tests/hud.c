...
int main(void){char lines[HUD_LINE_COUNT][160];
 ...
 rotation_inputs=CRAWL_RIGHTING|CRAWL_LEFT;hud_lines(lines);assert(strstr(lines[3],"Hold H: upright | Ctrl+J / K: left / right"));assert(strstr(lines[4],"UPRIGHT + LEFT"));
 rotation_inputs=CRAWL_RIGHT;hud_lines(lines);assert(strstr(lines[4],"RIGHT"));
 direction=1;hud_lines(lines);assert(strstr(lines[0],": 5.0 kN"));direction=-1;
 *(U*)G(0x7be0f4)=1;hud_lines(lines);assert(strstr(lines[0],"paused")&&strstr(lines[0],"0.0 kN"));assert(strstr(lines[2],"0 applying"));assert(strstr(lines[4],"none"));
 *(U*)G(0x7be0f4)=0;blocked=1;hud_lines(lines);assert(strstr(lines[0],"fault")&&strstr(lines[0],"0.0 kN"));
 ...
 crawl_hud=1;hud_render_end();assert(draws==6&&ends==1);*(int*)G(0x7a8960)=1920;*(int*)G(0x7a8964)=1080;hud_render_end();assert(draws==12&&ends==2);crawl_hud_left=1;hud_render_end();assert(draws==18&&ends==3);crawl_hud=0;hud_render_end();assert(draws==18&&ends==4);}
