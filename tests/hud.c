...
static U __fastcall fake_draw(void *font,int x,int y,U color,const WCHAR *text){assert(x==168&&y==436+draws*12&&color==0xffffffff&&text[0]);draws++;return 0;}
 ...
 *(U*)G(0x7be0f4)=0;blocked=1;hud_lines(lines);assert(strstr(lines[0],"fault")&&strstr(lines[0],"0.0 kN"));
 ...
 {U table[16]={0};U font=(U)table;table[6]=(U)fake_height;table[13]=(U)fake_draw;*(U*)G(0x7b64d4)=(U)&font;*(int*)G(0x7a8960)=640;*(int*)G(0x7a8964)=480;hud_end_original=fake_end;
 crawl_hud=1;hud_render_end();assert(draws==3&&ends==1);crawl_hud=0;hud_render_end();assert(draws==3&&ends==2);}
