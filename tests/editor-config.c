...
int main(int argc,char **argv){
 ...
 read_config();assert(config_valid&&!editor_windows&&!editor_free_tools&&!editor_idle_audio&&!editor_swap_keys&&!editor_unlimited_pan);
 for(bits=0;bits<32;bits++){
  ...
  assert(WritePrivateProfileStringW(L"Editors",L"UnlimitedMousePan",bits&16?L"true":L"false",ini));
  read_config();assert(config_valid&&editor_windows==!!(bits&1)&&editor_free_tools==!!(bits&2)&&editor_idle_audio==!!(bits&4)&&editor_swap_keys==!!(bits&8)&&editor_unlimited_pan==!!(bits&16));
 }
 ...
 puts("PASS editor configuration: missing options off, thirty-two independent combinations, custom keys, duplicate/reserved/invalid rejection, disabled binding isolation.");return 0;
}
