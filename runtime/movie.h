/* MIT. Startup movie skip and input restoration. Native rendering is untouched.
...
static U __fastcall movie_get_option(const WCHAR *name,U *value,U scope){
 ...
 movie_log("ShowLogo preference (0 means movie disabled)",value?*value:0);
 if(skip_startup_movie&&value){*value=0;movie_log("Startup movie skipped by NEMT (memory only)",0);}
 return result;
}
...
static U __fastcall movie_play(const char *path,U edx){
 U result;movie_log("PLAY entered",0);result=movie_original(path,edx);
...
static int install_movie_hook(void){
 if(!restore_movie_focus&&!skip_startup_movie)return 1;
 if(memcmp((void*)0x52fc32,"\x55\x8b\xec\x81\xec\x68\x01\x00\x00",9))return 0;
 startup_call(&movie_hooks[0],0x52fc29,0x52fc32,(U)movie_play);
 startup_call(&movie_hooks[1],0x52fbbe,0x403247,(U)movie_get_option);
