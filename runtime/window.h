 ...
 if(state==0){
  ...
  if(length<32750&&GetModuleFileNameW(NULL,root,MAX_PATH)&& (slash=wcsrchr(root,L'\\'))!=NULL){
   ...
   if(wcslen(root)+45<MAX_PATH&&supported_image()){
    ...
    if(!toolset_mode){
    if(config_valid && !apply_high_resolution())MessageBoxW(NULL,L"NEMT could not enable high-resolution compatibility on this Direct3D version. If MSTS fails above 2048 pixels, use a lower resolution or see the high-resolution guide in NEMT.",L"NEMT high-resolution compatibility",MB_OK|MB_ICONWARNING);
