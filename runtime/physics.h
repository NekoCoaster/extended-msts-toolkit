...
static int calc_pitch(float *omega,float *right,double scale,float *out){
 double u[3],pitch=0;int i;if(!finite_number(scale)||scale<0||scale>1||!unit_vector(right,u))return 0;
 for(i=0;i<3;i++){if(!finite_number(omega[i]))return 0;pitch+=omega[i]*u[i];}
 for(i=0;i<3;i++)out[i]=omega[i]-(1-scale)*pitch*u[i];return 1;
}
