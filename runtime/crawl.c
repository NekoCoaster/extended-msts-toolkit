...
#define G(a) (a)
#endif
enum {FRAME,EXIT,WHEEL,ROD,SPLIT,BODY,FORCE,DRAG,MANUAL_KEY,PITCH,VECTOR,CROSS,HOOK_COUNT};
#include "prologues.h"
...
static U rotation_inputs,control_id;
#include "crawl_controls.h"
...
static void update_frame(void){
 ...
 for(i=0;i<car_count;i++){Car *c=&cars[i];float right[3],up[3],omega[3],inverse[9];
  c->active=c->rotating=0;memset(c->dw,0,sizeof(c->dw));memset(c->dl,0,sizeof(c->dl));if(!c->engine||!c->derailed)continue;
  ...
  c->rotating=rotation_inputs!=0&&dt>0;
  if(c->rotating){
   for(j=0;j<3;j++){right[j]=rf((B*)c->body+0xc+j*4);up[j]=rf((B*)c->body+0x18+j*4);omega[j]=rf((B*)c->body+0x94+j*4);}
   memcpy(inverse,(void*)(c->body+0x64),sizeof(inverse));
   if(!calc_rotation(right,up,c->axis,omega,rotation_inputs&CRAWL_RIGHTING,
       !!(rotation_inputs&CRAWL_RIGHT)-!!(rotation_inputs&CRAWL_LEFT),dt,strength,c->dw)||
      !rotation_momentum(inverse,c->dw,c->dl)){fail("Invalid rotation parameters");return;}
   for(j=0;j<3;j++){double l=rf((B*)c->body+0x58+j*4)+c->dl[j],w=omega[j]+c->dw[j];
    if(!finite_number(l)||fabs(l)>3.4e38||!finite_number(w)||fabs(w)>3.4e38){fail("Invalid angular momentum");return;}}
  }
 ...
 for(i=0;i<car_count;i++){Car *c=&cars[i];if(!c->engine||!c->derailed||(!c->rotating&&!(c->dp[0]||c->dp[1]||c->dp[2])))continue;
  *((B*)c->body+0xf2)&=~8;
  for(j=0;j<3;j++){wf(c->body+0x4c+j*4,rf((B*)c->body+0x4c+j*4)+c->dp[j]);wf(c->body+0x88+j*4,c->velocity[j]+c->dv[j]);}
  if(c->rotating)for(j=0;j<3;j++){wf(c->body+0x58+j*4,rf((B*)c->body+0x58+j*4)+c->dl[j]);wf(c->body+0x94+j*4,rf((B*)c->body+0x94+j*4)+c->dw[j]);}
...
static void wake_body(U b,int force){Car *c;if(!eligible(b))return;c=find_car(ru((B*)b+0x11d));if((!force&&c->active)||(c->engine&&rotation_inputs&&strength)||friction_scale<1){*((B*)b+0xf2)&=~8;wakes++;}}
static void enter_effect(U id,Registers *r){
 ...
 if((id==PITCH||id==VECTOR||id==CROSS)&&!counter_tilt)return;
 if(id==PITCH){
  ...
  c=find_car(ru((B*)body+0x11d));
  /* A rolled local-up steering axis can coincide with suppressed world pitch.
     Let the native derivative turn freely while the player actively steers. */
  factor=c&&c->engine&&strength&&(!!(rotation_inputs&CRAWL_LEFT)!=!!(rotation_inputs&CRAWL_RIGHT))?1:friction_scale;
  if(!calc_pitch(omega,forward,factor,call->filtered)){fail("Invalid pitch input");return;}
 ...
 if(id==DRAG){
  ...
  body=r->ecx-offset;if(!eligible(body))return;factor=rf(&s[2]);if(!finite_number(factor)||factor>0){fail("Unexpected drag coefficient");return;}c=find_car(ru((B*)body+0x11d));wf((U)&s[2],factor*(offset==0x94&&c&&c->engine&&rotation_inputs&&strength?0:friction_scale));drag_calls++;return;
 }
 if(id==WHEEL||id==ROD){
...
static void write_status(void){
 ...
 do{before=status_sequence;if(before&1)continue;
  length=snprintf(text,sizeof(text),"{\"version\":2,\"runtime\":\"native\",\"phase\":\"%s\",\"enabled\":%s,\"strength\":%d,\"error\":\"%s\",\"frame\":%u,\"connected\":%u,\"activeEngines\":%u,\"controlType\":%u,\"throttle\":%.6f,\"impulses\":%u,\"wakes\":%u,\"dragCalls\":%u,\"wheelCalls\":%u,\"rodCalls\":%u,\"graphChanges\":%u}\n",phase,enabled?"true":"false",strength,last_error,frame_count,car_count,paused()?0:active_count,control_type,throttle,impulses,wakes,drag_calls,pitch_calls,wheel_calls,rod_calls,graph_changes);
...
static int start_native(void){
 ...
 if(crawl_requested){
  ...
  for(tries=WHEEL;tries<HOOK_COUNT;tries++)if(tries!=MANUAL_KEY){hooks[tries].gate=(U)&enabled;hooks[tries].pause=G(0x7be0f4);}
  ...
  /* Optional derivative hooks are the trailing three entries. */
  count=counter_tilt?HOOK_COUNT:PITCH;
 }
