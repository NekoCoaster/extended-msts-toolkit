...
subprocess.run([sys.argv[1],'-DWINVER=0x0501','-D_WIN32_WINNT=0x0501','-shared','-o',str(root/'DINPUT.dll'),str(root/'loader.c'),'-ladvapi32','-luser32'],check=True)
subprocess.run([sys.executable,str(root/'fix-exports.py'),str(root/'DINPUT.dll')],check=True)
subprocess.run([sys.executable,str(root.parent/'tools'/'check-win32-imports.py'),str(root/'DINPUT.dll')],check=True)
...
print('Built XP-baseline native DLL, audited imports and refreshed installation integrity hash.')
