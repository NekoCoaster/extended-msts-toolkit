...
static void hud_render_end(void){
 ...
 if(crawl_hud&&font){HudDrawFn draw=*(HudDrawFn*)(*(U*)font+0x34);HudHeightFn get_height=*(HudHeightFn*)(*(U*)font+0x18);height=get_height(font);
  if(height>0&&height<100&&screen_height>height*3+10){hud_lines(lines);y=screen_height-height*3-8;x=screen_width/4+8;
   for(i=0;i<3;i++){wide[0]=0;MultiByteToWideChar(0,0,lines[i],-1,wide,160);draw(font,x,y+i*height,0xffffffff,wide);}
  }
 }
 hud_end_original();
}
