/* MIT. Add six lines only at the end of the extended white F5 renderer. */
...
enum {HUD_LINE_COUNT=6};
static void hud_lines(char lines[HUD_LINE_COUNT][160]){
 const char *status="disabled",*reverser="neutral";U powered=0,applying=0,i;double force=0,t=0;int boost=strength,is_paused=0;U rotation=0;char keys[3][40]={"Space",";","\'"};int known=0;
 if(crawl_requested){status="waiting for activity";
  if(crawl_state_ready){EnterCriticalSection(&state_lock);is_paused=paused();status=blocked?"fault":is_paused?"paused":enabled?"active":have_sim?"ready":"armed";boost=strength;t=clamp_throttle(throttle);reverser=direction>0?"forward":direction<0?"reverse":"neutral";
   ...
   memcpy(keys,crawl_shortcuts,sizeof(keys));known=crawl_shortcut_ctl!=0;
   if(enabled&&!blocked&&!is_paused)rotation=rotation_inputs;
 ...
 snprintf(lines[3],160,"%s %s: upright | %s / %s: left / right",known?"Hold":"Default keys:",keys[0],keys[1],keys[2]);
 snprintf(lines[4],160,"Rotation input: %s%s%s",rotation&CRAWL_RIGHTING?"UPRIGHT":"",(rotation&CRAWL_RIGHTING)&&(rotation&(CRAWL_LEFT|CRAWL_RIGHT))?" + ":"",rotation&CRAWL_LEFT?"LEFT":rotation&CRAWL_RIGHT?"RIGHT":rotation?"":"none");
 snprintf(lines[5],160,"Press \\: derail the controlled train");
}
static void hud_render_end(void){
 void *font=*(void**)G(0x7b64d4);int screen_height=*(int*)G(0x7a8964),screen_width=*(int*)G(0x7a8960),height,x,y,i;char lines[HUD_LINE_COUNT][160];WCHAR wide[160];
 if(crawl_hud&&font){HudDrawFn draw=*(HudDrawFn*)(*(U*)font+0x34);HudHeightFn get_height=*(HudHeightFn*)(*(U*)font+0x18);HudWidthFn get_width=*(HudWidthFn*)(*(U*)font+0x24);height=get_height(font);
  if(height>0&&height<100&&screen_height>height*HUD_LINE_COUNT+10){hud_lines(lines);y=screen_height-height*HUD_LINE_COUNT-8;
   for(i=0;i<HUD_LINE_COUNT;i++){wide[0]=0;MultiByteToWideChar(0,0,lines[i],-1,wide,160);x=crawl_hud_left?8:screen_width-8-get_width(font,wide);if(x<8)x=8;draw(font,x,y+i*height,0xffffffff,wide);}
  }
 }
 hud_end_original();
}
