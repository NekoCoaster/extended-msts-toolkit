...
Start with [the findings and coverage checkpoint](CHECKPOINT.md), [the flat inventory](INVENTORY.md), and its detailed machine-readable counterpart, `inventory.json`. The current 902 entries include 304 direct-read candidates/structures, 23 derived candidates, 68 cab channels and 507 static configuration paths (474 leaf and 37 mixed paths with four overlaps). These counts are not independent verified live values. Configuration leaf cleanup and further semantic validation remain open.

Recent findings distinguish station cue state from recorded departure, establish connection endpoint calculations, and qualify stored body velocities against moving player/AI observations. Track-following orientation can change while stored angular velocity stays zero. The player position/velocity discrepancy remains unresolved; the derived endpoint calculation is not yet a validated trajectory measurement. See STATION-TIME-FINDINGS.md, COUPLING-GEOMETRY-FINDINGS.md and MOTION-DIFFERENCE-FINDINGS.md.
