...
$exportArgs=@('C:\dev\Codex\2026-09-10\msts-precipitation\ghidra-projects','MSTS','-process','train.exe','-noanalysis','-readOnly','-scriptPath',$PSScriptRoot,'-postScript','TelemetrySteamWrites.java',$output)
...
if(-not(Test-Path (Join-Path $output 'steam-offset-writes.tsv'))) {throw 'Export missing; inspect console.log'}
