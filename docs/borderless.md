...
Enable **Enable borderless windowed mode** in NEMT.exe to remove the window frame. With it unchecked, MSTS retains its normal launch mode; supply `-vm:w` for an ordinary framed window. Apply and restart MSTS.
...
| --- | --- | --- |
| `train.exe` | Centered borderless window | Native MSTS default (no mode override) |
...
Use resolutions supported by your installation and display. Existing windowed shortcuts work with either checkbox state. Launches without a display-mode argument default to windowed mode only when borderless mode is enabled.
