...
Enable **Allow connected engines to crawl after derailment**, Apply with MSTS closed, then restart. The form includes the HUD automatically; there is no separate HUD checkbox. Press F5 through the red pages to reach MSTS Bin's white extended display. Six extra lines show state and multiplier, applied thrust in kN, throttle/regulator and reverser, applying/connected powered-engine counts, mapped control keys, rotational input and the manual-derail shortcut. The red pages are unchanged.
