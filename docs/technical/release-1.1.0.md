...
The 1.1.0 release preparation changes presentation and documentation without rebuilding or changing the tested runtime DLL. Packaging verifies DLL integrity, relative documentation links, file hashes, archive contents and excluded private/game files. Development now uses `dev`; a pull request into `main` is the approval boundary. Preparing this version does not publish a release or create a tag.
