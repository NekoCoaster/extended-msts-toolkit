...
No additional detours are installed. The existing frame callback reads held native keyboard bindings from IOM's device list (`0x8299A0`), device keyboard bitset (`+0x24`), and binding table (`+0x18`, keyboard offset `+0x10`). It checks native modifier masks, disabled bindings/actions/listeners, mode `0x829980`, foreground focus and pause. Traversals are bounded; failed reads produce no rotational input. Keyboard remapping follows the registered callbacks and controller contexts: horn/whistle `0x489A25` with steam controller `+0x368` or diesel/electric `+0x1C4`; train brake increase/decrease `0x4898DD`/`0x489987` with steam `+0xE0` or diesel/electric `+0x10C`. Brake position is not used, so a held key still steers at the lever's end stop. Joystick and mouse-operated cab controls are not rotational inputs.
