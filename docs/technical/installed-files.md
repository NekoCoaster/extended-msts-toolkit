...
| `NEMT/startup.log` (optional) | Deep startup/activity/API diagnostic trace; created only when Startup.WriteLog is enabled. |
...
The optional extended F5 HUD reads in-memory telemetry and adds no files or file writes. Deep logging retains its handle for activity loading and selected API/exception events.
