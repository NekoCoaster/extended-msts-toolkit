...
param([Parameter(Position=0)][Alias('Path')][string]$ExePath,
 ...
 [ValidateRange(0,100)][int]$Strength=10,[switch]$FixCabDials,[switch]$BackgroundAudio,[switch]$IgnoreRedSignal)
...
function Invoke-Options {
 ...
 $printStatus='false';$centerWindowed='true';$maxLogSizeKB=8192;$maxBackupLogs=0;$cabNeedle='false';$restoreMovieFocus='true';$systemMovieDecoder='true';$separateMovieWindow='true';$bindMovieOutput='true'
 foreach($record in $records){
  ...
  if(Test-Path -LiteralPath $ini){
   ...
   foreach($line in [IO.File]::ReadAllLines($ini)){
    ...
    elseif($section -ieq 'Startup' -and $line -match '^\s*BindMovieOutput\s*=\s*(.*?)\s*$'){$bindMovieOutput=if($Matches[1] -imatch '^(true|1)$'){'true'}else{'false'}}
    elseif($section -ieq 'Startup' -and $line -match '^\s*SeparateMovieWindow\s*=\s*(.*?)\s*$'){$separateMovieWindow=if($Matches[1] -imatch '^(true|1)$'){'true'}else{'false'}}
 ...
 try{
  if($install){
   ...
   $config="[Startup]`r`nVerboseLoading=$($VerboseLoading.ToString().ToLowerInvariant())`r`nWriteLog=$($StartupLog.ToString().ToLowerInvariant())`r`nUnlockFPS=$($UnlockFPS.ToString().ToLowerInvariant())`r`nBindMovieOutput=$bindMovieOutput`r`nSeparateMovieWindow=$separateMovieWindow`r`nUseSystemMovieDecoder=$systemMovieDecoder`r`nRestoreMovieFocus=$restoreMovieFocus`r`nMaxLogSizeKB=$maxLogSizeKB`r`nMaxBackupLogs=$maxBackupLogs`r`n`r`n[Window]`r`nEnabled=$($WindowFeatures.ToString().ToLowerInvariant())`r`nCenterWindowed=$centerWindowed`r`n`r`n[Derailment]`r`nPreventActivityEnd=$($Timeout.ToString().ToLowerInvariant())`r`nUnlockCameras=$($Camera.ToString().ToLowerInvariant())`r`nEnableCrawl=$($CrawlMode.ToString().ToLowerInvariant())`r`nCrawlStrength=$Thrust`r`n`r`n[Diagnostics]`r`nWriteStatusJson=$printStatus`r`nShowCrawlHUD=$($CrawlHUD.ToString().ToLowerInvariant())`r`nCrawlHUDAnchor=$CrawlHUDAnchor`r`n"
...
function Show-Options([string]$InitialPath){
 ...
 $load={param([string]$p)
  $ui.info=$null;$ui.loading=$true;$apply.Enabled=$false;$restore.Enabled=$false;$pathText.Text=$p;$cabBox.Enabled=$false;$cabBox.Checked=$false;$backgroundBox.Checked=$false;$redBox.Checked=$false
 ...
 $save={param([bool]$clear)
  ...
  try{$t=if($clear){$false}else{$timeout.Checked};$c=if($clear){$false}else{$camera.Checked};$r=if($clear){$false}else{$crawl.Checked};$w=if($clear){$false}else{$window.Checked}
   $result=Invoke-Options -Path $ui.info.Path -Timeout $t -Camera $c -CrawlMode $r -Thrust $slider.Value -ExpectedHash $ui.info.SHA256 -WindowFeatures $w -VerboseLoading ((-not $clear) -and $verbose.Checked) -StartupLog ((-not $clear) -and $startupLogBox.Checked) -UnlockFPS ((-not $clear) -and $fps.Checked) -CrawlHUD $r -CrawlHUDAnchor $(if($anchor.SelectedIndex -eq 1){"BottomLeft"}else{"BottomRight"}) -FixCabDials ((-not $clear) -and $cabBox.Enabled -and $cabBox.Checked) -BackgroundAudio ((-not $clear) -and $backgroundBox.Checked) -IgnoreRedSignal ((-not $clear) -and $redBox.Checked) -Confirm:$false
